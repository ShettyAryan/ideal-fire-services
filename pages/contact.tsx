  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="Contact Ideal Fire Services for fire safety audits, installation, and AMC in Mumbai." />
      </Head>
      <Navbar />
      ...
      <section className="bg-[#0f0505] py-8 border-b border-white/5">
        <div className="max-w-7xl mx-auto px-5 sm:px-8">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {[
              { icon: 'location_on', label: 'Office', value: 'Plot 45, Goregaon West, Mumbai 400104' },
              { icon: 'phone', label: 'Phone', value: '+91 22 1234 5678', href: 'tel:+912212345678' },
              { icon: 'mail', label: 'Email', value: 'info@idealfire.in', href: 'mailto:info@idealfire.in' },
            ].map((item, i) => (
              <div key={i} className="flex items-center gap-4 py-3">
                ...
                <div>
                  ...
                  {item.href ? (
                    <a href={item.href} className="text-white font-semibold text-sm hover:text-primary transition-colors">
                      {item.value}
                    </a>
                  ) : (
        ...
        <div className="max-w-7xl mx-auto px-5 sm:px-8">
          <div className="grid lg:grid-cols-12 gap-12">
            ...
            <div className="lg:col-span-7 reveal-left">
              <div className="section-label mb-3">
                <span className="text-primary font-bold uppercase tracking-widest text-sm">Enquire Now</span>
              </div>
              <h2 className="font-display text-4xl font-black text-slate-900 uppercase mb-8">Send Us a Message</h2>
              ...
              {submitted ? (
                <div className="bg-white rounded-2xl p-12 text-center shadow-sm border border-slate-100">
                  ...
                  <h3 className="font-display text-3xl font-black text-slate-900 uppercase mb-3">Message Sent!</h3>
                  <p className="text-slate-500 mb-6">Our team will get back to you within 24 business hours.</p>
                  <button
                ...
                <form onSubmit={handleSubmit} className="bg-white rounded-2xl p-8 shadow-sm border border-slate-100 space-y-5">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div>
                      <label className="text-xs font-black uppercase tracking-widest text-slate-500 block mb-2">Phone *</label>
                      <input
                        type="tel"
                        required
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div>
                      <label className="text-xs font-black uppercase tracking-widest text-slate-500 block mb-2">Company / Building</label>
                      <input
                        type="text"
                  ...
                  <div>
                    <label className="text-xs font-black uppercase tracking-widest text-slate-500 block mb-2">Service Required</label>
                    <select
                  ...
                  <div>
                    <label className="text-xs font-black uppercase tracking-widest text-slate-500 block mb-2">Message</label>
                    <textarea
                      rows={4}
                  ...
                  <p className="text-center text-xs text-slate-400">Your data is safe and handled per our privacy policy.</p>
                </form>
              )}
            </div>
            ...
            <aside className="lg:col-span-5 space-y-6">
              {/* Working hours */}
              <div className="bg-white rounded-2xl p-8 shadow-sm border border-slate-100 reveal" style={{ transitionDelay: '0.1s' }}>
                <div className="flex items-center gap-3 mb-6">
                  <div className="w-10 h-10 bg-primary/10 rounded-xl flex items-center justify-center">
                    <span className="material-symbols-outlined text-primary">schedule</span>
                  </div>
                  <h3 className="font-display text-2xl font-black text-slate-900 uppercase">Working Hours</h3>
                </div>
                <div className="space-y-3">
                  {[
                    { day: 'Monday – Friday', hours: '9:30 AM – 6:00 PM', open: true },
                    { day: 'Saturday', hours: '9:30 AM – 4:00 PM', open: true },
                    { day: 'Sunday', hours: 'Emergency Only', open: false },
                  ].map((item) => (
                    <div key={item.day} className="flex items-center justify-between py-2.5 border-b border-slate-50 last:border-0">
                      <span className="text-sm font-semibold text-slate-700">{item.day}</span>
                      <div className="flex items-center gap-2">
                        <span className={`w-2 h-2 rounded-full ${item.open ? 'bg-green-400' : 'bg-red-400'}`} />
                        <span className="text-sm text-slate-500">{item.hours}</span>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {/* Quick assistance */}
              <div className="bg-[#0f0505] rounded-2xl p-8 relative overflow-hidden reveal" style={{ transitionDelay: '0.2s' }}>
                <div className="absolute -right-8 -bottom-8 opacity-5">
                  <span className="material-symbols-outlined" style={{ fontSize: '160px' }}>verified_user</span>
                </div>
                <h3 className="font-display text-2xl font-black text-white uppercase mb-6 relative z-10">Quick Assistance</h3>
                <div className="space-y-5 relative z-10">
                  {[
                    { icon: 'description', title: 'Fire Safety Audit', desc: 'Request a professional audit for your premises.' },
                    { icon: 'engineering', title: 'Emergency Response', desc: 'On-site within 4 hours across Mumbai & Thane.' },
                    { icon: 'phone_in_talk', title: 'Direct Hotline', desc: '+91 22 1234 5678 — always staffed.' },
                  ].map((item) => (
              ...
              {/* FAQ */}
              <div className="bg-white rounded-2xl p-8 shadow-sm border border-slate-100 reveal" style={{ transitionDelay: '0.3s' }}>
                <h3 className="font-display text-2xl font-black text-slate-900 uppercase mb-6">Common Questions</h3>
                <div className="space-y-4">
                  {[
                    { q: 'How fast is your site visit response?', a: 'Our team initiates site visits within 24–48 business hours of the request.' },
                    { q: 'Do you provide Form B certificates?', a: 'Yes, we provide full Form B compliance certificates required by Mumbai Fire Brigade.' },
                    { q: 'Which areas do you cover?', a: 'We cover all of Mumbai, Navi Mumbai, and Thane with our trained technician teams.' },
                  ].map((faq, i) => (
              ...
              {/* Certification badge */}
              <div className="flex items-center gap-4 p-5 bg-primary/5 border border-primary/15 rounded-2xl reveal" style={{ transitionDelay: '0.4s' }}>
                <span className="material-symbols-outlined text-primary text-4xl" style={{ fontVariationSettings: "'FILL' 1" }}>workspace_premium</span>
                <div>
                  <p className="font-black text-slate-900 uppercase tracking-wider text-xs">Certified Agency</p>
                  <p className="text-[10px] text-slate-500 mt-0.5">Government Licensed Fire Safety Agency — Maharashtra</p>
                </div>
              </div>
            </aside>
          </div>
      ...
      <section className="relative h-96 overflow-hidden">
        ...
        <div className="relative h-full max-w-7xl mx-auto px-5 sm:px-8 flex items-end pb-8">
          <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-sm border border-slate-100">
            <h4 className="font-black text-primary uppercase tracking-wider text-xs mb-1">Visit Our Office</h4>
            <p className="font-bold text-slate-900 text-sm mb-2">Ideal Fire Services HQ</p>
            <p className="text-xs text-slate-500 mb-4 leading-relaxed">Plot 45, Industrial Estate, Off Link Road,<br />Goregaon West, Mumbai 400104</p>
            <a
